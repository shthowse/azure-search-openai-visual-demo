...
"If you cannot answer using the sources below, say you don't know. Return just the answer without any input texts"
    ...
    def __init__(self, search_client: SearchClient, openai_deployment: str, chatgpt_model: str, openai_gptv_deployment: str, gptv_model: str, embedding_deployment: str, sourcepage_field: str, content_field: str):
    ...
    def download_blob_as_base64(self, file_path: str) -> str:
        base_name, _ = os.path.splitext(file_path)
        blob = blob_container.get_blob_client(base_name + ".png").download_blob()
    
        if not blob.properties or not blob.properties.has_key("content_settings"):
            return
        return base64.b64encode(blob.readall()).decode('utf-8')

    def generate_image_embeddings(self, text):
    ...
    def temp_search_fn_multi_vector(self, query_text, filter=None, query_type=None, query_language=None, query_speller=None, semantic_configuration_name=None, top=None, query_caption=None, vectors=None):
        ...
        headers = {  
            "Content-Type": "application/json",  
            "api-key": os.environ['AZURE_SEARCH_SERVICE_KEY']
        }  
        data = {  
            "search": query_text,
            "filter":filter,
        ...
        return requests.post(endpoint, params=params, headers=headers, json=data).json()['value']

    # Place holder function. We will replace request with openai sdk when available
    def gptv_request(self, data):
        ...
        combined_text = "".join(choice["text"] for choice in json.loads(response.text)["choices"])
        return combined_text
    
    def trim_embedding(self, embedding):
        """Format the embedding list to show the first 2 items followed by the count of the remaining items."""
        return '[{}, {} ...+{} more]'.format(embedding[0], embedding[1], len(embedding) - 2) if len(embedding) > 2 else embedding
    ...
    async def run(self, q: str, overrides: dict[str, Any]) -> ApproachResult:
        ...
        if use_gptv:
            data = {
            "transcript": [
                {"type": "text", "data": self.system_chat_template_gptv},
                {"type": "text", "data": q }
             ],
            "max_tokens": 1024,
            ...
            for source, text_result in results:
                data['transcript'].append({"type": "text", "data": ": ".join([source, text_result])}) if include_gtpV_text else None
                data['transcript'].append({"type": "image", "data": self.download_blob_as_base64(source)}) if include_gtpV_images else None
                
            chat_content = self.gptv_request(data)  
            result_dict = {t: [item["data"] for item in data['transcript'] if item["type"] == t] for t in ["text", "image"]}
            ...
            return ApproachResult(
                ...
                [
                    ThoughtStep("Search Query", query_text, {"vectorFields": vector_fields , "semanticCaptions": use_semantic_captions}),
